...
The benchmark measures real-world agent performance by running concrete code navigation tasks with Claude — once using indexer commands, once using only grep/find/cat — and comparing total tokens consumed, tool calls, turns, and answer correctness.

### Run the benchmark

```bash
# Preview tasks without calling the API (free)
...
For each task, the benchmark runs a Claude agent in two modes and tracks:

| Metric | What it captures |
|---|---|
| **Input tokens** | Total prompt tokens across all turns (system + messages + tool results) |
| **Output tokens** | Total completion tokens across all turns |
| **Tool calls** | Number of bash invocations the agent needed |
| **Turns** | Conversation round-trips to reach an answer |
| **Correctness** | Whether the answer contains the ground truth file paths / symbol names |
| **Wall time** | End-to-end elapsed time per task |

### Task types

Tasks are auto-generated by introspecting the project's index, so ground truth is always accurate:

| Category | Example question | Ground truth source |
|---|---|---|
| `symbol_lookup` | "Find where `HandleRequest` is defined" | `indexer search` |
| `caller_trace` | "List all files that call `NewServer`" | `indexer callers` |
| `file_understanding` | "What functions are defined in `handler.go`?" | `indexer skeleton` |
| `text_search` | "Find all files containing `config`" | `indexer grep` |
| `file_discovery` | "Find all files with `test` in their name" | `indexer find` |
